...
class TestApi(AcctMixin, ApiTokenMixin, BaseTestCase):
    ...
    def _add_model_permission(self, user, model, actions):
        """actions must be one of: 'view', 'add', 'change', 'delete'"""
    ...
    def test_radius_user_group_list(self):
        ...
        with self.subTest("Create RADIUS user group"):
            ...
            self.assertEqual(response.status_code, 201)
            data = response.data
            self.assertEqual(data["user"], str(member.id))
            self.assertEqual(data["group"], str(group.id))
            self.assertEqual(data["username"], member.username)
            self.assertEqual(data["groupname"], group.name)

        with self.subTest("Create RadiusUserGroup with other organization"):
    ...
    def test_radius_user_group_detail(self):
        ...
        self._add_model_permission(user, RadiusUserGroup, ["view", "change", "delete"])
        ...
        with self.subTest("GET operation"):
            ...
            data = response.json()
            self.assertEqual(data["user"], str(member.id))
            self.assertEqual(data["group"], str(user_group.group.id))
            self.assertEqual(data["id"], str(user_group.id))
        ...
        with self.subTest("DELETE operation"):
            ...
            self.assertEqual(
                RadiusUserGroup.objects.filter(pk=user_group.pk).count(), 0
            )
